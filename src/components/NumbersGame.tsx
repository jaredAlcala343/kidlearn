...
import {
  speak, speakCountSequence, playSparkleTone, playWrongTone,
  speakCelebration, playCountBlip, initAudio
} from '@/lib/sounds'
...
function ExploreTab() {
  ...
  const [currentNum, setCurrentNum]   = useState(0)   // live counter display
  ...
  const handleSelect = useCallback((item: typeof NUMBERS[0]) => {
    ...
    setCurrentNum(0)
    setActive(item)
    speak(`${item.number}. ${item.label}`, 0.76, 1.1, true)
  }, [])
  ...
  const handleCountTogether = useCallback(() => {
    ...
    // speakCountSequence fires onStep(n) exactly when it's about to say n
    // so visual and audio are perfectly in sync
    const cancel = speakCountSequence(
      active.number,
      (n: number) => {
        // Highlight emoji index n-1 (0-based)
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'28px', justifyContent:'center' }}>
        {NUMBERS.map(item => (
          <button key={item.number} onClick={() => handleSelect(item)}
            style={{
      ...
      <div style={{ borderRadius:'28px', padding:'28px', backgroundColor:active.color+'13', border:`3px solid ${active.color}44`, marginBottom:'20px' }}>
        ...
        <div style={{
          ...
          backgroundColor:'white', border:`2px solid ${active.color}22`,
          marginBottom:'18px', minHeight:'80px', position:'relative',
        }}>
          {emojis.map((e, i) => (
            <span key={i} style={{
              ...
              filter: highlighted.length===0 || highlighted.includes(i)
                ? 'none'
          ...
          {/* Live count badge over the grid */}
          {counting && currentNum > 0 && (
            <div className="pop" style={{
              ...
              boxShadow:`0 3px 10px ${active.color}88`,
            }}>
              {currentNum}
            </div>
          )}
        ...
        <div style={{ display:'flex', gap:'10px', justifyContent:'center', flexWrap:'wrap' }}>
          <button
            onClick={() => { initAudio(); speak(`${active.number}. ${active.label}`, 0.76, 1.1, true) }}
          ...
          <button
            onClick={handleCountTogether}
            style={{
              padding:'10px 22px', borderRadius:'14px', fontWeight:700, fontSize:'14px',
              backgroundColor: counting ? '#E8A87C' : active.color,
              color:'white', border:'none', cursor:'pointer',
              boxShadow:`0 4px 0 ${counting ? '#C47D47' : active.color+'BB'}`,
...
function generateOptions(correct: number): number[] {
  ...
  for (const n of near) { if (opts.size < 4) opts.add(n) }
  while (opts.size < 4) {
...
function QuizTab() {
  ...
  const [options,  setOptions]   = useState(() => {
    ...
    return generateOptions(NUMBERS[idx].number)
  })
  ...
  const nextRound = useCallback(() => {
    ...
    setItemIdx(next)
    setOptions(generateOptions(NUMBERS[next].number))
    setCounted([]); setSelected(null); setIsCorrect(null)
    setAnimKey(k=>k+1)
  }, [])
  ...
  const handleEmojiTap = useCallback((i: number) => {
    ...
    // Speak the number briefly
    speak(String(next.length), 0.75, 1.2, true)
  }, [counted, selected])
  ...
  const handleOption = useCallback((num: number) => {
    ...
    if (correct) {
      ...
      if (ns%5===0) { speakCelebration(); setShowConf(true); setTimeout(()=>setShowConf(false),3000) }
      else setTimeout(()=>speak('¡Correcto!',0.88,1.15,true),200)
    ...
    <div>
      ...
      <div key={animKey} className="slide-in">
        <p style={{textAlign:'center',fontSize:'17px',fontWeight:800,color:'#6B6B6B',marginBottom:'16px'}}>
          ¿Cuántos {item.emoji} hay?{' '}
          <span style={{color:'#3D3D3D',fontWeight:600}}>Toca cada uno para contar</span>
        </p>
        ...
        <div style={{borderRadius:'24px',padding:'20px',backgroundColor:item.color+'14',border:`3px solid ${item.color}44`,marginBottom:'16px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'10px'}}>
            ...
            {counted.length>0 && (
              <button onClick={()=>setCounted([])}
                style={{fontSize:'12px',fontWeight:700,padding:'4px 12px',borderRadius:'12px',border:'none',cursor:'pointer',backgroundColor:item.color+'33',color:item.color}}>
                ↺ Reset
              </button>
            )}
          </div>
          <div style={{display:'flex',flexWrap:'wrap',gap:'6px',justifyContent:'center'}}>
            {emojis.map((e,i)=>(
              <button key={i} onClick={()=>handleEmojiTap(i)}
                style={{
        ...
        {isCorrect && (
          <div className="pop" style={{marginTop:'16px',textAlign:'center',padding:'16px',borderRadius:'20px',backgroundColor:'#EEF7EA',border:'2px solid #B8D4AC'}}>
            <p style={{margin:0,fontSize:'20px',fontWeight:900,color:'#1A4A10'}}>
              ✅ ¡Correcto! Son <span style={{color:item.color}}>{item.label}</span>
            </p>
          </div>
        )}
      </div>
        ...
        {(['explore','quiz'] as const).map(id=>(
          <button key={id} onClick={()=>setTab(id)}
            style={{
              ...
              backgroundColor:tab===id?'#8BAF7C':'transparent',
