...
import {
  playLetter, playLetterWord, playWord,
  playSparkleTone, playWrongTone, playCelebrationVoice,
  playCorrect, playTryAgain, initAudio
} from '@/lib/sounds'
...
function buildQuiz(): QuizCard {
  const pool = LETTERS.filter(l => l.letter !== lastLetter)
  ...
  const distractors = shuffle(LETTERS.filter(x => x.letter !== correctLetter.letter)).slice(0, 3)
  return { correctLetter, entry, options: shuffle([correctLetter, ...distractors]) }
}
...
function ExploreTab() {
  ...
  const handleTap = useCallback((item: LetterItem) => {
    ...
    setActive(item); setActiveWord(entry)
    setBounce(item.letter)
    playLetterWord(item.letter, entry.word)
  ...
  const cycleWord = useCallback(() => {
    ...
    const next = others.length ? others[Math.floor(Math.random() * others.length)] : active.words[0]
    setActiveWord(next)
    playLetterWord(active.letter, next.word)
  }, [active, activeWord])
  ...
  return (
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(76px, 1fr))', gap:'10px', marginBottom:'24px' }}>
        {LETTERS.map(item => (
          <button key={item.letter} onClick={() => handleTap(item)} style={{
            ...
            <span style={{ fontSize:'clamp(0.85rem,2.5vw,1.2rem)', lineHeight:1 }}>{item.words[0].emoji}</span>
          </button>
        ))}
      </div>

      {active && activeWord ? (
        <div className="pop" style={{ borderRadius:'24px', padding:'24px 28px', backgroundColor:active.color+'16', border:`3px solid ${active.color}55` }}>
          <div style={{ display:'flex', gap:'20px', alignItems:'center', flexWrap:'wrap' }}>
            <div style={{ width:'84px', height:'84px', borderRadius:'20px', flexShrink:0, backgroundColor:active.color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'clamp(2.8rem,8vw,3.5rem)', fontWeight:900, color:'white', boxShadow:`0 5px 0 ${active.color}99` }}>
              {active.letter}
            </div>
            <div style={{ flex:1, minWidth:'160px' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'6px' }}>
                ...
                <p style={{ margin:0, fontSize:'clamp(1.4rem,4vw,2.2rem)', fontWeight:900, color:'#3D3D3D', letterSpacing:'2px' }}>
                  <span style={{ color:active.color }}>{active.letter}</span>{activeWord.word.slice(1).toLowerCase()}
                </p>
              </div>
              ...
              <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                <button onClick={() => playLetterWord(active.letter, activeWord.word)}
            ...
            <div style={{ width:'100%', display:'flex', gap:'8px', flexWrap:'wrap', marginTop:'4px' }}>
              {active.words.map(w => (
                <button key={w.word} onClick={() => { setActiveWord(w); playLetterWord(active.letter, w.word) }}
                  style={{ padding:'4px 10px', borderRadius:'10px', fontSize:'12px', fontWeight:700, backgroundColor:activeWord.word===w.word?active.color:active.color+'22', color:activeWord.word===w.word?'white':active.color, border:'none', cursor:'pointer' }}>
                  {w.emoji} {w.word}
                </button>
              ))}
            </div>
        ...
        <div style={{ textAlign:'center', padding:'28px', borderRadius:'20px', backgroundColor:'#F5F2EE' }}>
          <p style={{ margin:0, fontSize:'16px', color:'#6B6B6B', fontWeight:600 }}>👆 Toca cualquier letra para explorarla</p>
        </div>
      )}
    </div>
  )
...
function QuizTab() {
  ...
  useEffect(() => {
    const t = setTimeout(() => { initAudio(); playWord(quiz.entry.word) }, 500)
    return () => clearTimeout(t)
  }, [animKey]) // eslint-disable-line react-hooks/exhaustive-deps

  const nextQuestion = useCallback(() => {
    setQuiz(buildQuiz()); setSelected(null); setIsCorrect(null); setAnimKey(k => k + 1)
  }, [])
  ...
  const handleAnswer = useCallback((letter: string) => {
    ...
    if (correct) {
      ...
      if (ns % 5 === 0) {
        ...
        setShowConf(true); setTimeout(() => setShowConf(false), 3000)
      } else {
        playCorrect()
      }
  ...
  return (
    <div>
      ...
      <div key={animKey} className="slide-in">
        ...
        <div onClick={() => playWord(quiz.entry.word)} style={{
          borderRadius:'24px', padding:'28px 20px', marginBottom:'24px', cursor:'pointer',
          backgroundColor:quiz.correctLetter.color+'14', border:`3px solid ${quiz.correctLetter.color}44`,
          ...
          {quiz.options.map(opt => {
            ...
            return (
              <button key={opt.letter} onClick={() => handleAnswer(opt.letter)}
                style={{ padding:'20px 12px', borderRadius:'20px', border:`3px solid ${border}`, backgroundColor:bg, color:col, fontWeight:900, fontSize:'clamp(2rem,8vw,3.5rem)', boxShadow:shadow, cursor:'pointer', transition:'all 0.15s', display:'flex', alignItems:'center', justifyContent:'center', gap:'4px' }}
      ...
      <div style={{ display:'flex', gap:'8px', marginBottom:'28px', padding:'6px', borderRadius:'20px', backgroundColor:'#EEF6FB' }}>
        {(['explore','quiz'] as const).map(id => (
          <button key={id} onClick={() => setTab(id)}
            style={{ flex:1, padding:'12px 8px', borderRadius:'14px', border:'none', cursor:'pointer', backgroundColor:tab===id?'#7EB8D4':'transparent', color:tab===id?'white':'#6B6B6B', fontWeight:tab===id?800:600, transition:'all 0.2s', display:'flex', flexDirection:'column', alignItems:'center', gap:'2px' }}>
